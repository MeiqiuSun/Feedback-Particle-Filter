...
from single_frequency import Figure1, Figure2
...
if __name__ == "__main__":
    ...
    # amp = signal_type.amp
    # feedback_particle_filter = FPF(number_of_particles=Np, model=Model2(amp=amp, freq_range=freq_range, min_sigma_B=min_sigma_B, min_sigma_W=min_sigma_W), \
    #                                galerkin=Galerkin24(), sigma_B=signal_type.sigma_B, sigma_W=signal_type.sigma_W, dt=signal_type.dt)

    amp_range = [[0.5,1.5],[2.5,3.5]]
    feedback_particle_filter = FPF(number_of_particles=Np, model=Model4(amp_range=amp_range, freq_range=freq_range, min_sigma_B=min_sigma_B, min_sigma_W=min_sigma_W), \
                                   galerkin=Galerkin44(), sigma_B=signal_type.sigma_B, sigma_W=signal_type.sigma_W, dt=signal_type.dt)
    # feedback_particle_filter = FPF(number_of_particles=Np, model=Model4(amp_range=amp_range, freq_range=freq_range, min_sigma_B=min_sigma_B, min_sigma_W=min_sigma_W),\
    #                                galerkin=Galerkin48(), sigma_B=signal_type.sigma_B, sigma_W=signal_type.sigma_W, dt=signal_type.dt)
    filtered_signal = feedback_particle_filter.run(signal.Y)

    fontsize = 20
    fig_property = Struct(fontsize=fontsize, show=False, plot_signal=False, plot_X=False, plot_histogram=False, plot_c=False)
